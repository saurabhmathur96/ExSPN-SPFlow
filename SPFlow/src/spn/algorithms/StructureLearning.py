...
def learn_structure(
    ...
    while tasks:
        ...
        if operation == Operation.REMOVE_UNINFORMATIVE_FEATURES:
            ...
            # print (data_slices[0])
            # print (data_slices[1])
            # node.data_slices = copy.deepcopy(data_slices)
            # print (data_slices[0][0].shape)
            # print (data_slices[0][1])
            # [:, scope_slice]
            
            # y = np.concatenate([np.zeros(len(data_slice)) + j for j, (data_slice, scope_slice, proportion) in enumerate(data_slices)])
            # X = np.concatenate([data_slice for data_slice, scope_slice, proportion in data_slices])
            # node.X = X
            # node.y = y
            node.data_slices = [data_slice for data_slice, scope_slice, proportion in data_slices]
            

            
            # feature_importances = compute_feature_importances(X, y)
