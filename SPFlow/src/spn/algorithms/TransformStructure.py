...
def Prune(node, contract_single_parents=True):
    ...
    while len(nodes) > 0:
        ...
        while i < len(n.children):
            ...
            if n_type == type(c):
                if is_sum:
                    n.data_slices = n.data_slices[:i] + n.data_slices[i+1:] + c.data_slices
    ...
    for n in get_nodes_by_type(node, (Sum)):
        n.y = np.concatenate([np.zeros(len(data_slice)) + j 
                            for j, data_slice in enumerate(n.data_slices)])
        n.X = np.concatenate(n.data_slices)
        del n.data_slices
    assign_ids(node)
    v, err = is_valid(node)
    assert v, err
